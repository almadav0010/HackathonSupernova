  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${redirectUrl}/auth/callback?redirect=${redirect}`,
        },
      })
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-text-secondary mt-2">Welcome back to collaborative learning</p>
        </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-text-secondary">
            ...
            <Link href="/auth/signup" className="text-text-primary font-bold hover:underline">
              Sign Up
            </Link>
          </p>
        </div>
